...
export default {
  ...
  methods: {
    ...
    drawChart(option) {
      ...
      try {
        // 如果配置中有 grid，则使用配置的 grid，否则使用默认值
        if (option.grid && typeof option.grid === 'object') {
          this.grid = {
            top: option.grid.top !== undefined ? option.grid.top : this.grid.top,
            right: option.grid.right !== undefined ? option.grid.right : this.grid.right,
            bottom: option.grid.bottom !== undefined ? option.grid.bottom : this.grid.bottom,
            left: option.grid.left !== undefined ? option.grid.left : this.grid.left,
          };
        }
        ...
        // 如果有标题，调整网格顶部边距（但不超过配置的值）
        if (titleHeight > 0) {
          const configuredTop = option.grid && option.grid.top !== undefined ? option.grid.top : null;
          if (configuredTop === null) {
            // 如果没有配置 top，则自动调整
    ...
    drawBars(series, xAxisData, minY, maxY, adjustedYMin, adjustedYMax, xAxisPadding = 10) {
      ...
      series.forEach((serie, index) => {
        ...
        points.forEach((point, pointIndex) => {
          ...
          // 绘制标签
          if (point.showLabel && point.labelConfig) {
            this.drawBarLabel(point, positionInfo);
          }
        });
      });
     ...
     * @created 2025-07-28
     */
    drawBarLabel(point, positionInfo) {
      if (!point.labelConfig || !this.ctx) return;
      
      const label = point.labelConfig;
      const position = label.position || 'top'; // top, inside, bottom
      const labelColor = label.color || '#333';
      const fontSize = label.fontSize || 10;
      const labelText = String(point.value);
      
      // 计算标签位置
      let labelX = point.x + point.barWidth / 2; // 标签X位置：柱子中心
      let labelY;
      
      // 计算柱子的实际绘制位置
      const drawY = point.value >= 0 ? point.y : (positionInfo.isStack ? point.y : point.zeroY);
      const barTop = drawY;
      const barBottom = drawY + point.barHeight;
      ...
      // 如果是 inside 位置且柱子高度太小，调整位置到顶部
      if (position === 'inside' && point.barHeight < 20) {
        labelY = barTop - 5;
      }
