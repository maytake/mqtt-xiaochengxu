    ...
    <view class="page-bg">
      ...
      <view class="card settings-card">
        ...
        <view class="list">
          ...
          <!-- 出皂时间 -->
          <view class="list-item">
            <view :class="['label', { dot: soapTimeWarn }]" @click="handlePidTip('soapTime')">出皂时间</view>
            <view class="extra value" @click="openSoapTimePopup">
              <text>{{ soapTime }}</text>
              <text class="font_family m-arrow arrow">&#xe60d;</text>
            </view>
          </view>
      ...
      <up-popup :show="showSoapTimePopup" :round="10" mode="bottom" :closeOnClickOverlay="false"
        ...
        <view class="popup-content">
          <view class="title-row">
            <view class="action-btn action-cancel" @click="closeSoapPopup">取消</view>
            <view class="title">出皂时间</view>
...
const tempSoapSeconds = ref(0.1);
const soapTime = computed(() => `${soapSeconds.value }S`);

const showSoapTimePopup = ref(false);
const openSoapTimePopup = () => {

  tempSoapSeconds.value = soapSeconds.value;
  showSoapTimePopup.value = true;
};
const closeSoapPopup = () => {
